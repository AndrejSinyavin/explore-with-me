...
public interface StatsRepository extends JpaRepository<EndpointHitEntity, Long> {
    @Query("""
            select new ru.practicum.ewm.statservice.commondto.ViewStatsDto(e.app, e.uri, count(e.ip))
    ...
    @Query("""
            select new ru.practicum.ewm.statservice.commondto.ViewStatsDto(e.app, e.uri, count(distinct e.ip))
            from EndpointHitEntity e
            where e.timestamp >= :begin and e.timestamp <= :end and (e.uri in :uris)
            group by e.app, e.uri
            order by e.app, e.uri""")
